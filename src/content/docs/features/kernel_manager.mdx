...
|:---------------:	|:--------------------:	|:---------------------------------------------------------------------------------------------------------------------------:	|
| **Low Latency** 	| `-s 5000 -S 500 -l 5000 -m performance` 	| Meant to lower latency at the cost of throughput. Suitable for real-time applications like Audio Processing and Multimedia. 	|
...
#### Flash
   ...
   Profile Name     |  Command-line Flags   |                                                         Description                                                           |
|:---------------:  |:--------------------: |:---------------------------------------------------------------------------------------------------------------------------:  |
| **Low Latency**   | `-m performance -w -C 0`   | Meant to lower latency at the cost of throughput. Suitable for real-time applications like Audio Processing and Multimedia.   |
| **Gaming**        | `-m all`      | Optimizes for high performance in games. Prioritizes P-cores over E-cores on Intel CPUs and CCDs on Ryzen X3D CPUs.           |
| **Power Save**    | `-m powersave -I 10000 -t 10000 -s 10000 -S 1000`        | Prioritizes power efficiency. Favors less performant cores (e.g E-cores on Intel).                                            |
...
|   Profile Name    |  Command-line Flags   |                                                         Description                                                           |
|:---------------:  |:--------------------: |:---------------------------------------------------------------------------------------------------------------------------:  |
| **Gaming**   | `-f 5000 -s 5000`   | Improves gaming performance by increasing the amount of times (in ticks) the scheduler checks for the slice expiration on running tasks    |
| **Power Save**        | `-f 50 -p`      | Improves power efficiency by reducing the frequency of time slice expiration checks and tries to keep the tasks running on the same CPU without migration.          |
| **Low Latency**    | `-f 5000 -s 1000`        | Similar to the gaming profile but with a further reduced slice.                                            |
| **Server**        | `-f 100`                | Reduced slice expiration checks to a bare minimum to improve throughput at the cost of responsiveness.